...
def get_gold_context_pos(gold_context: str, retrieved_contexts: List[str]) -> int:
    """
    Get the position of the gold context in the retrieved contexts.
    
    Args:
        gold_context (str): The gold context to find.
        retrieved_contexts (List[str]): List of retrieved contexts.
        
    Returns:
        int: Position (1-based index) of the gold context in the retrieved contexts, or -1 if not found.
    """
    # Check if gold_context is a string or a list
    gold_contexts = [gold_context] if isinstance(gold_context, str) else gold_context
    
    if not gold_contexts or not retrieved_contexts:
        return -1
    
    lowest_position = len(retrieved_contexts) + 10  # Start with a position larger than any possible index
    
    for gold in gold_contexts:
        # Normalize the gold context for comparison
        normalized_gold = gold.lower().strip() if gold else ""
        if not normalized_gold:
            continue
            
        # Check for matches
        for i, context in enumerate(retrieved_contexts):
            normalized_context = context['content'].lower().strip() if context else ""
            if normalized_gold in normalized_context or normalized_context in normalized_gold:
                position = i + 1  # +1 for 1-based index
                lowest_position = min(lowest_position, position)
    
    # Return -1 if no match was found
    return lowest_position if lowest_position != (len(retrieved_contexts) + 10) else -1

def is_answer_correct(gold_answer: list[str], generated_answer: str) -> tuple[bool, int]:
    """
    Determine if the generated answer is correct based on the gold answer.
    
    Args:
        gold_answer (list[str]): The list of expected correct answers.
        generated_answer (str): The answer generated by the model.
        
    Returns:
        tuple[bool, int]: A tuple containing:
            - bool: True if the generated answer is correct, False otherwise
            - int: Position of the matching gold answer (0-indexed) if a match is found,
                   -1 if no match found
    """
    ...
    gen_answer_norm = generated_answer.lower().strip()
    
    # Check if any of the gold answers match
    for i, answer in enumerate(gold_answer):
        if not answer:  # Skip empty answers in the list
            continue
            
        # Special case for 'any'
        if answer.lower().strip() == 'any' and len(gen_answer_norm) > 0:
            return True, i
            
        if answer.lower().strip() in gen_answer_norm:
            return True, i  # Return 0-indexed position of matching answer
