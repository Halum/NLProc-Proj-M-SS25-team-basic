...
def get_gold_context_pos(gold_context: str, retrieved_contexts: List[str]) -> int:
    ...
    normalized_gold = gold_context.lower().strip() if gold_context else ""
    print(f"Normalized gold context: '{normalized_gold}'")  # Debugging output
    
    if not normalized_gold or not retrieved_contexts:
        return -1
        
    # If no exact match, check for substring
...
def is_answer_correct(gold_answer: str, generated_answer: str) -> bool:
    """
    Determine if the generated answer is correct based on the gold answer.
    
    Args:
        gold_answer (str): The expected correct answer.
    ...
    print(f"Gold answer: '{gold_answer}'")  # Debugging output
    print(f"Generated answer: '{generated_answer}'")  # Debugging output
        
    # Simple substring match - could be enhanced with NLP techniques
    return gold_answer.lower() in generated_answer.lower()